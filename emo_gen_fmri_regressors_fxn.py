...
# sset paths
behav_path = (
    '/Users/lillyel-said/Desktop/stanford/scripts/jupyter_notebooks/anger_good')
curr_dir = os.getcwd()
...
def extract_emoperception_conditions(behav_data_path, file, output_txt_path):
    ...
    for ix, trial in enumerate(trials):
        index_with_cor.append(
            [ix, trial[1].split(':')[3].split("['")[1].split("']")[0]])
        percent_emotion = trial[1].split('{')[:][1].split(',')[4]
        emotion = trial[1].split('{')[:][1].split(',')[0]
        combined = (emotion.split("': u'")[
                    1] + '_' + percent_emotion.split(': ')[1]).replace("'", '')
    ...
    write_arr_tofile(happy_0, os.path.join(
        behav_dir, output_dir, filename + '_happy0.txt'))
    write_arr_tofile(happy_25, os.path.join(
        behav_dir, output_dir, filename + '_happy25.txt'))
    write_arr_tofile(happy_50, os.path.join(
        behav_dir, output_dir, filename + '_happy50.txt'))
    write_arr_tofile(happy_75, os.path.join(
        behav_dir, output_dir, filename + '_happy75.txt'))
    write_arr_tofile(happy_100, os.path.join(
        behav_dir, output_dir, filename + '_happy100.txt'))
